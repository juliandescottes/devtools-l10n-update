...
function transformConsoleAPICallResource(consoleMessageResource) {
  ...
  switch (type) {
    case "clear":
      ...
      parameters = [l10n.getString("consoleCleared")];
      break;
    case "count":
    case "countReset":
      ...
      if (!counter) {
        ...
        messageText = l10n.getFormatString(counter.error, [counter.label]);
        level = MESSAGE_LEVEL.WARN;
        parameters = null;
      } else {
        const label = counter.label
          ? counter.label
          : l10n.getString("noCounterLabel");
    ...
    case "time":
      ...
      if (timer && timer.error) {
        messageText = l10n.getFormatString(timer.error, [timer.name]);
        level = MESSAGE_LEVEL.WARN;
      } else {
    ...
    case "timeEnd":
      if (timer && timer.error) {
        ...
        if (type === "timeEnd") {
          messageText = l10n.getFormatString("console.timeEnd", [
            timer.name,
            duration,
          ]);
          parameters = null;
          ...
          parameters = [
            l10n.getFormatString("timeLog", [timer.name, duration]),
            ...rest,
          ];
        }
      } else {
    ...
    case "group":
      ...
      if (parameters.length === 0) {
        parameters = [l10n.getString("noGroupLabel")];
      }
      break;
    case "groupCollapsed":
...
function transformNavigationMessagePacket(packet) {
  ...
  return new ConsoleMessage({
    ...
    messageText: l10n.getFormatString("webconsole.navigated", [url]),
...
function transformEvaluationResultPacket(packet) {
  ...
  if (hasException) {
    ...
    try {
      exceptionMessage = l10n.getFormatString(
...
function getWarningGroupLabel(firstMessage) {
  ...
  if (isCookieSameSiteMessage(firstMessage)) {
    if (Services.prefs.getBoolPref("network.cookie.sameSite.laxByDefault")) {
      return l10n.getString("webconsole.group.cookieSameSiteLaxByDefaultEnabled2");
    }
    return l10n.getString("webconsole.group.cookieSameSiteLaxByDefaultDisabled2");
  }
