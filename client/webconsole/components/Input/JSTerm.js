...
class JSTerm extends Component {
        ...
        extraKeys: {
          ...
          [Editor.accel("S")]: () => {
            ...
            return saveAs(window, data, suggestedName, [
              {
                pattern: "*.js",
                label: l10n.getString("webconsole.input.openJavaScriptFileFilter"),
              },
            ]);
          },
  ...
  async _openFile() {
    ...
    fp.init(
      ...
      l10n.getString("webconsole.input.openJavaScriptFile"),
      Ci.nsIFilePicker.modeOpen
    );

    // Append file filters
    fp.appendFilter(
      l10n.getString("webconsole.input.openJavaScriptFileFilter"),
      "*.js"
    );
  ...
  renderOpenEditorButton() {
    ...
    return dom.button({
      ...
      title: l10n.getFormatString("webconsole.input.openEditorButton.tooltip2", [
  ...
  renderEditorOnboarding() {
    ...
    // We deliberately use getString, and not getFormatString, because we want keyboard
    // shortcuts to be wrapped in their own span.
    const label = l10n.getString("webconsole.input.editor.onboarding.label");
    let [prefix, suffix] = label.split("%1$S");
    suffix = suffix.split("%2$S");

    const enterString = l10n.getString("webconsole.enterKey");
    ...
    return dom.header(
      ...
      dom.button(
        ...
        l10n.getString("webconsole.input.editor.onboarding.dismiss.label")
      )
    );
  }
