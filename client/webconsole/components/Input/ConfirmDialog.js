...
class ConfirmDialog extends Component {
  ...
  render() {
    ...
    // We deliberately use getString, and not getFormatString, because we want getterName to
    // be wrapped in its own span.
    const description = l10n.getString("webconsole.confirmDialog.getter.label");
    const [descriptionPrefix, descriptionSuffix] = description.split("%S");

    const closeButtonTooltip = l10n.getFormatString(
      "webconsole.confirmDialog.getter.closeButton.tooltip",
      ["Esc"]
    );
    const invokeButtonLabel = l10n.getFormatString(
    ...
    const learnMoreElement = dom.a(
      ...
      l10n.getString("webConsoleMoreInfoLabel")
    );
