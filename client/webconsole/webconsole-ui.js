...
class WebConsoleUI {
  ...
  logWarningAboutReplacedAPI() {
    return this.hud.currentTarget.logWarningInPage(
      l10n.getString("ConsoleAPIDisabled"),
      "ConsoleAPIDisabled"
    );
  }
  ...
  _initShortcuts() {
    ...
    if (AppConstants.platform === "macosx") {
      const alternativaClearShortcut = l10n.getString(
        "webconsole.clear.alternativeKeyOSX"
      );
      shortcuts.on(alternativaClearShortcut, event =>
        this.clearOutput(true, event)
      );
      clearShortcut = l10n.getString("webconsole.clear.keyOSX");
    } else {
      clearShortcut = l10n.getString("webconsole.clear.key");
    }
    ...
    if (this.isBrowserConsole) {
      ...
      this.window.focus();
      shortcuts.on(
        l10n.getString("webconsole.close.key"),
