...
exports.register = function(window, shortcuts) {
  ...
  shortcuts.on(L10N.getString("toolbox.zoomIn.key"), zoomIn);
  const zoomIn2 = L10N.getString("toolbox.zoomIn2.key");
  if (zoomIn2) {
    shortcuts.on(zoomIn2, zoomIn);
  }

  shortcuts.on(L10N.getString("toolbox.zoomOut.key"), zoomOut);
  const zoomOut2 = L10N.getString("toolbox.zoomOut2.key");
  if (zoomOut2) {
    shortcuts.on(zoomOut2, zoomOut);
  }

  shortcuts.on(L10N.getString("toolbox.zoomReset.key"), zoomReset);
  const zoomReset2 = L10N.getString("toolbox.zoomReset2.key");
  if (zoomReset2) {
