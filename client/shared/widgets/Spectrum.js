...
class Spectrum {
  constructor(parentEl, rgb) {
    ...
    <section class="spectrum-color-picker">
      <div class="spectrum-color spectrum-box"
           tabindex="0"
           role="slider"
           title="${L10N.getString("colorPickerTooltip.spectrumDraggerTitle")}"
  ...
  `;
    ...
    eyedropper.setAttribute(
      "aria-label",
      L10N.getString("colorPickerTooltip.eyedropperTitle")
    );
    ...
    learnMore.title = L10N.getString("accessibility.learnMore");
    this.element
  ...
  get colorName() {
    ...
    return minDeltaE === 0
      ? colorName
      : L10N.getFormatString("colorPickerTooltip.colorNameTitle", colorName);
  }
  ...
  createSlider(sliderType, onSliderMove) {
    ...
    slider.title = L10N.getString(`colorPickerTooltip.${sliderType}SliderTitle`);
  ...
  updateContrastLabel(isLargeText) {
    if (!isLargeText) {
      this.contrastLabel.textContent = L10N.getString(
    ...
    const largeTextStr = L10N.getString("accessibility.contrast.large.text");
    const contrastLabelStr = L10N.getFormatString(
    ...
    largeTextIndicator.title = L10N.getString(
  ...
  updateContrast() {
    ...
    if (error) {
      ...
      const contrastValEl = isRange
        ? this.contrastValueMin
        : this.contrastValue;
      contrastValEl.textContent = L10N.getString("accessibility.contrast.error");
      contrastValEl.title = L10N.getString(
