...
const filterList = [
  ...
  {
    name: "drop-shadow",
    placeholder: L10N.getString("dropShadowPlaceholder"),
    type: "string",
  },
  {
    name: "grayscale",
...
CSSFilterEditorWidget.prototype = {
  _initMarkup: function() {
    ...
    filterListPlaceholder.textContent = L10N.getString(
    ...
    addFilter.textContent = L10N.getString("addNewFilterButton");
    ...
    this.togglePresets.textContent = L10N.getString("presetsToggleButton");
    ...
    this.addPresetInput.setAttribute(
      "placeholder",
      L10N.getString("newPresetPlaceholder")
    );
    ...
    this.addPresetButton.textContent = L10N.getString("savePresetButton");
  ...
  _buildFilterItemMarkup: function() {
    ...
    drag.title = L10N.getString("dragHandleTooltipText");
  ...
  render: function() {
    if (!this.filters.length) {
      // eslint-disable-next-line no-unsanitized/property
      this.filterList.innerHTML = `<p> ${L10N.getString("emptyFilterList")} <br />
                                 ${L10N.getString("addUsingList")} </p>`;
      this.emit("render");
      return;
    }
    ...
    for (const filter of this.filters) {
      ...
      if (def.type !== "string") {
        ...
        label.title = L10N.getString("labelDragTooltipText");
      } else {
  ...
  renderPresets: function() {
    this.getPresets().then(presets => {
      ...
      if (!presets || !presets.length) {
        // eslint-disable-next-line no-unsanitized/property
        this.presetList.innerHTML = `<p>${L10N.getString("emptyPresetList")}</p>`;
        this.emit("render");
        return;
      }
