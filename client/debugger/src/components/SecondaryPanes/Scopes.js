...
class Scopes extends PureComponent {
  ...
  onContextMenu = (event, item) => {
    ...
    if (!item.contents || item.contents.watchpoint) {
      const removeWatchpointLabel = L10N.getString("watchpoints.removeWatchpoint");
    ...
    const addSetWatchpointLabel = L10N.getString("watchpoints.setWatchpoint");
    const addGetWatchpointLabel = L10N.getString("watchpoints.getWatchpoint");
    const addGetOrSetWatchpointLabel = L10N.getString(
      "watchpoints.getOrSetWatchpoint"
    );
    const watchpointsSubmenuLabel = L10N.getString("watchpoints.submenu");
  ...
  renderWatchpointButton = item => {
    ...
    return (
      <button
        ...
        title={L10N.getString("watchpoints.removeWatchpointTooltip")}
        onClick={e => {
          e.stopPropagation();
  ...
  renderScopesList() {
    ...
    let stateText = L10N.getString("scopes.notPaused");
    if (this.props.isPaused) {
      if (isLoading) {
        stateText = L10N.getString("loadingText");
      } else {
        stateText = L10N.getString("scopes.notAvailable");
      }
    }
