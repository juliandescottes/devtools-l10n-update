  ...
  renderResults = () => {
    ...
    const msg =
      status === statusType.fetching
        ? L10N.getString("loadingText")
        : L10N.getString("projectTextSearch.noResults");
    return <div className="no-result-msg absolute-center">{msg}</div>;
  };

  renderSummary = () => {
    if (this.props.query !== "") {
      const resultsSummaryString = L10N.getString("sourceSearch.resultsSummary2");
  ...
  renderInput() {
    ...
    return (
      <SearchInput
        ...
        placeholder={L10N.getString("projectTextSearch.placeholder")}
        size="big"
