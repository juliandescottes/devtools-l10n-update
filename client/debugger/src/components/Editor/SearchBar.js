...
function getShortcuts() {
  const searchAgainKey = L10N.getString("sourceSearch.search.again.key3");
  const searchAgainPrevKey = L10N.getString("sourceSearch.search.againPrev.key3");
  const searchKey = L10N.getString("sourceSearch.search.key2");
...
class SearchBar extends Component {
  ...
  buildSummaryMsg() {
    ...
    if (count == 0) {
      return L10N.getString("editor.noResultsFound");
    }

    if (index == -1) {
      const resultsSummaryString = L10N.getString("sourceSearch.resultsSummary1");
      return PluralForm.get(count, resultsSummaryString).replace("#1", count);
    }

    const searchResultsString = L10N.getString("editor.searchResults1");
    ...
    return (
      <div className="search-modifiers">
        ...
        <span className="search-type-name">
          {L10N.getString("symbolSearch.searchModifier.modifiersLabel")}
        </span>
        <SearchModBtn
          modVal="regexMatch"
          className="regex-match-btn"
          svgName="regex-match"
          tooltip={L10N.getString("symbolSearch.searchModifier.regex")}
        />
        <SearchModBtn
          modVal="caseSensitive"
          className="case-sensitive-btn"
          svgName="case-match"
          tooltip={L10N.getString("symbolSearch.searchModifier.caseSensitive")}
        />
        <SearchModBtn
          modVal="wholeWord"
          className="whole-word-btn"
          svgName="whole-word-match"
          tooltip={L10N.getString("symbolSearch.searchModifier.wholeWord")}
        />
      </div>
    );
  };
    ...
    return (
      <div className="search-bar">
        <SearchInput
          ...
          count={count}
          placeholder={L10N.getString("sourceSearch.search.placeholder2")}
