...
export class WelcomeBox extends Component {
  render() {
    ...
    const searchProjectShortcut = formatKeyShortcut(
      L10N.getString("projectTextSearch.key")
    );

    const allShortcutsShortcut = formatKeyShortcut(
      L10N.getString("allShortcut.key")
    );

    const allShortcutsLabel = L10N.getString("welcome.allShortcuts");
    const searchSourcesLabel = L10N.getString("welcome.search2").substring(2);
    const searchProjectLabel = L10N.getString("welcome.findInFiles2").substring(2);
