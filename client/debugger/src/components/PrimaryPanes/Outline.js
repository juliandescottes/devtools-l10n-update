...
export class Outline extends Component {
  ...
  onContextMenu(event, func) {
    ...
    const copyFunctionItem = {
      ...
      label: L10N.getString("copyFunction.label"),
      accesskey: L10N.getString("copyFunction.accesskey"),
  ...
  renderPlaceholder() {
    const placeholderMessage = this.props.selectedSource
      ? L10N.getString("outline.noFunctions")
      : L10N.getString("outline.noFileSelected");
  ...
  renderLoading() {
    return (
      <div className="outline-pane-info">{L10N.getString("loadingText")}</div>
    );
  }
  ...
  renderFooter() {
    return (
      <div className="outline-footer">
        ...
        >
          {L10N.getString("outline.sortLabel")}
        </button>
      </div>
    );
  }
