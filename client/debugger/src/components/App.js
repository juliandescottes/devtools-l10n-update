...
class App extends Component {
  ...
  componentDidMount() {
    ...
    shortcuts.on(L10N.getString("symbolSearch.search.key2"), e =>
      this.toggleQuickOpenModal(e, "@")
    );

    const searchKeys = [
      L10N.getString("sources.search.key2"),
      L10N.getString("sources.search.alt.key"),
    ];
    searchKeys.forEach(key => shortcuts.on(key, this.toggleQuickOpenModal));

    shortcuts.on(L10N.getString("gotoLineModal.key3"), e =>
      this.toggleQuickOpenModal(e, ":")
    );
  ...
  componentWillUnmount() {
    ...
    shortcuts.off(
      L10N.getString("symbolSearch.search.key2"),
    ...
    shortcuts.off(L10N.getString("gotoLineModal.key3"), this.toggleQuickOpenModal);
    ...
    return (
      <div className={classnames("debugger")}>
        <AppErrorBoundary
          ...
          panel={L10N.getString("ToolboxDebugger.label")}
        >
          <A11yIntention>
            {this.renderLayout()}
