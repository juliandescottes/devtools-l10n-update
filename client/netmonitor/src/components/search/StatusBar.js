...
class StatusBar extends Component {
  ...
  getSearchStatusDoneLabel(lines, files) {
    const matchingLines = PluralForm.get(
      lines,
      L10N.getString("netmonitor.search.status.labels.matchingLines")
    ).replace("#1", lines);
    const matchingFiles = PluralForm.get(
      files,
      L10N.getString("netmonitor.search.status.labels.fileCount")
    ).replace("#1", files);

    return L10N.getFormatString(
  ...
  renderStatus() {
    ...
    switch (status) {
      case SEARCH_STATUS.FETCHING:
        return L10N.getString("netmonitor.search.status.labels.fetching");
      case SEARCH_STATUS.DONE:
        return this.getSearchStatusDoneLabel(resultsCount, resourceCount);
      case SEARCH_STATUS.ERROR:
        return L10N.getString("netmonitor.search.status.labels.error");
      case SEARCH_STATUS.CANCELED:
        return L10N.getString("netmonitor.search.status.labels.canceled");
      default:
        return "";
    }
  }
