...
const JSON_SCOPE_NAME = L10N.getString("jsonScopeName");
const JSON_FILTER_TEXT = L10N.getString("jsonFilterText");
const RESPONSE_PAYLOAD = L10N.getString("responsePayload");
const RAW_RESPONSE_PAYLOAD = L10N.getString("netmonitor.response.raw");
const HTML_RESPONSE = L10N.getString("netmonitor.response.html");
const RESPONSE_EMPTY_TEXT = L10N.getString("responseEmptyText");
const RESPONSE_TRUNCATED = L10N.getString("responseTruncated");
...
class ResponsePanel extends Component {
  ...
  renderCORSBlockedReason(blockedReason) {
    ...
    const messageText = L10N.getFormatString(
      "netmonitor.headers.blockedByCORS",
      blockedMessage
    );

    const learnMoreTooltip = L10N.getString(
  ...
  render() {
    ...
    if (json) {
      if (jsonpCallback) {
        responsePayloadLabel = L10N.getFormatString(
          "jsonpScopeName",
          jsonpCallback
        );
      } else {
