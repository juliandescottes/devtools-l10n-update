...
const BACK_BUTTON = L10N.getString("netmonitor.backButton");
const CHARTS_CACHE_ENABLED = L10N.getString("charts.cacheEnabled");
const CHARTS_CACHE_DISABLED = L10N.getString("charts.cacheDisabled");
const CHARTS_LEARN_MORE = L10N.getString("charts.learnMore");
  ...
  createChart({ id, title, data }) {
    ...
    const chart = Chart.PieTable(document, {
      ...
      header: {
        cached: "",
        count: "",
        label: L10N.getString("charts.type"),
        size: L10N.getString("charts.size"),
        transferredSize: L10N.getString("charts.transferred"),
        time: L10N.getString("charts.time"),
        nonBlockingTime: L10N.getString("charts.nonBlockingTime"),
      },
      data,
      strings: {
        size: value =>
          L10N.getFormatString("charts.sizeKB", getSizeWithDecimals(value / 1024)),
        transferredSize: value =>
          L10N.getFormatString(
            "charts.transferredSizeKB",
            getSizeWithDecimals(value / 1024)
          ),
        time: value =>
          L10N.getFormatString("charts.totalS", getTimeWithDecimals(value / 1000)),
        nonBlockingTime: value =>
          L10N.getFormatString("charts.totalS", getTimeWithDecimals(value / 1000)),
      },
      totals: {
        cached: total => L10N.getFormatString("charts.totalCached", total),
        count: total => L10N.getFormatString("charts.totalCount", total),
        size: total =>
          L10N.getFormatString(
            "charts.totalSize",
        ...
        time: total => {
          ...
          return PluralForm.get(
            seconds,
            L10N.getString("charts.totalSeconds")
          ).replace("#1", string);
        },
        nonBlockingTime: total => {
          ...
          return PluralForm.get(
            seconds,
            L10N.getString("charts.totalSecondsNonBlocking")
          ).replace("#1", string);
        },
      },
      sorted: true,
