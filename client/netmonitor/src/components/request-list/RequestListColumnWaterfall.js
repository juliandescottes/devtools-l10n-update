...
class RequestListColumnWaterfall extends Component {
  ...
  timingTooltip() {
    ...
    if (eventTimings) {
      for (const key of TIMING_KEYS) {
        ...
        if (width > 0) {
          tooltip.push(
            L10N.getFormatString("netmonitor.waterfall.tooltip." + key, width)
          );
        }
      }
    }
    ...
    if (typeof totalTime === "number") {
      tooltip.push(
        L10N.getFormatString("netmonitor.waterfall.tooltip.total", totalTime)
      );
    }

    return tooltip.join(L10N.getString("netmonitor.waterfall.tooltip.separator"));
  }
  ...
  timingBoxes() {
    ...
    if (typeof totalTime === "number") {
      ...
      const title = L10N.getFormatString("networkMenu.totalMS2", totalTime);
      boxes.push(
        div(
          {
            key: "total",
