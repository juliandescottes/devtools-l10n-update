...
Client.defaultSendOOB = ({ authResult, oob }) => {
  ...
  const title = L10N.getString("clientSendOOBTitle");
  const header = L10N.getString("clientSendOOBHeader");
  const hashMsg = L10N.getFormatString("clientSendOOBHash", oob.sha256);
  const token = oob.sha256.replace(/:/g, "").toLowerCase() + oob.k;
  const tokenMsg = L10N.getFormatString("clientSendOOBToken", token);
  const msg = `${header}\n\n${hashMsg}\n${tokenMsg}`;
...
Server.defaultAllowConnection = ({ client, server }) => {
  const title = L10N.getString("remoteIncomingPromptTitle");
  const header = L10N.getString("remoteIncomingPromptHeader");
  const clientEndpoint = `${client.host}:${client.port}`;
  const clientMsg = L10N.getFormatString(
    "remoteIncomingPromptClientEndpoint",
    clientEndpoint
  );
  const serverEndpoint = `${server.host}:${server.port}`;
  const serverMsg = L10N.getFormatString(
    "remoteIncomingPromptServerEndpoint",
    serverEndpoint
  );
  const footer = L10N.getString("remoteIncomingPromptFooter");
  const msg = `${header}\n\n${clientMsg}\n${serverMsg}\n\n${footer}`;
  const disableButton = L10N.getString("remoteIncomingPromptDisable");
  const prompt = Services.prompt;
  const flags =
...
Server.defaultReceiveOOB = () => {
  const title = L10N.getString("serverReceiveOOBTitle");
  const msg = L10N.getString("serverReceiveOOBBody");
